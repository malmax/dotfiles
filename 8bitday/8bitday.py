...
# except ImportError:
#    print("Couldn't find ConfigParser")
#    import configparser as cp
...
class Weather(object):
    ...
    def __init__(self, cached=False):
        ...
        self._api_key = self.config.get("default2", "api_key")
        self._lat = self.config.get("default2", "latitude")
        self._lon = self.config.get("default2", "longitude")
	...
	req = "https://api.darksky.net/forecast/{}/{},{}".format(self._api_key, self._lat, self._lon)
	r = requests.get(req)
        return r.json()
    ...
    def get_weather(self):
        #print(self.weather['currently'])
        if self.weather['currently']['icon'] == 'clear_day':
            return 'sunny'
        return "sunny_no_sun"
...
""".format(filename)
    ...
    #fileStr = os.path.join(base_dir(),filename)
    subprocess.call(["/usr/bin/feh", "--bg-scale", filename])

def base_dir():
    return resource_filename('8bitday', 'img')
...
def main():
    ...
    set_wallpaper(os.path.join(base_dir(), w.get_weather(), get_file_name(now, w)))
